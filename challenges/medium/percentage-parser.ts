...
type ParseSign<Str extends string> = Str extends `${infer Sign}${string}` ? Sign extends '+' | '-' ? Sign : '' : ''
type ParsePercent<Str extends string> = Str extends `${string}%` ? '%' : ''
type ParseNumber<Str extends string> = Str extends `${ParseSign<Str>}${infer Num}${ParsePercent<Str>}` ? Num : ''
type PercentageParser<Str extends string> = [ParseSign<Str>, ParseNumber<Str>, ParsePercent<Str>]
